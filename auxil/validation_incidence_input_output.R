...
analysis <- "mortality_calibration"

## Incidence ## ----

tt <- fread(paste0("/media/php-workstation/Storage_1/IMPACT_Storage/outputs/", analysis, "/summaries/incd_scaled_up.csv.gz")
)[, `:=` (year = year + 2000)]
...
e <- fst::read_fst("/home/php-workstation/Schreibtisch/IMPACT/2022_SSB_Tax_Model_Germany/Internal_Validation/disease_epi_l.fst", as.data.table = TRUE)
...
cowplot::ggsave2(paste0("validation_i_o_incidence_", i, "_", analysis, ".png"), width = 16, height = 9, units = "cm", 
                 scale = 2, dpi = 300, path = "/home/php-workstation/Schreibtisch/IMPACT/2022_SSB_Tax_Model_Germany/Internal_Validation/input_output/plots/")
...
tt <- fread(paste0("/media/php-workstation/Storage_1/IMPACT_Storage/outputs/", analysis, "/summaries/prvl_scaled_up.csv.gz")
)[, `:=` (year = year + 2000)]
tt[, grep("cms", names(tt), value = TRUE) := NULL]
...
e[, disease := gsub("_prevalence_rates", "", disease)]
e[, disease := gsub("value_", "", disease)]
...
for(i in unique(d$disease)){
  ...
  ggplot(d[disease == i], aes(x = agegrp, y = `prvl_rate_50.0%`, ymin = `prvl_rate_2.5%`,
    ...
    ggtitle("Comparison of input and output prevalence rates") +
    expand_limits(y = 0) +
    theme(legend.title = element_blank(), legend.position = "none")
