...
synthpops <- grep(pattern = ".fst", list.files("media/php-workstation/Storage 1/IMPACT_Storage/inputs/synthpop/"), value = TRUE)

ids <- sample(1:length(synthpops), 500, replace = FALSE)

dat <- foreach(i = seq(synthpops)[ids],
               .packages = c("fst", "data.table")) %dopar% {
                 
                 xx <- read_fst(paste0("media/php-workstation/Storage 1/IMPACT_Storage/inputs/synthpop/", synthpops[i]), as.data.table = TRUE)[
                   year >= 13, .(year, age, sex,
                                 bmi_curr_xps, ssb_curr_xps, juice_curr_xps,
                                 sugar_per_ssb, sugar_per_juice, wt)]
...
fwrite(bmi_synthpop_table, "media/php-workstation/Storage 1/IMPACT_Storage/PRIMEtime_export/bmi_synthpop_input_2013_2035.csv", sep = ";", dec = ".")
...
fwrite(ssb_synthpop_table, "media/php-workstation/Storage 1/IMPACT_Storage/PRIMEtime_export/ssb_synthpop_input_2013.csv", sep = ";", dec = ".")
...
fwrite(juice_synthpop_table, "media/php-workstation/Storage 1/IMPACT_Storage/PRIMEtime_export/juice_synthpop_input_2013.csv", sep = ";", dec = ".")
...
fwrite(ssb_sugar_table, "media/php-workstation/Storage 1/IMPACT_Storage/PRIMEtime_export/ssb_sugar_input_2013.csv", sep = ";", dec = ".")
...
fwrite(juice_sugar_table, "media/php-workstation/Storage 1/IMPACT_Storage/PRIMEtime_export/juice_sugar_input_2013.csv", sep = ";", dec = ".")
