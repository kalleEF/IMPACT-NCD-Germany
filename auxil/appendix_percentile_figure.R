...
sp <- sp[age %in% c(20, 80) & sex == "women" & year == 14]

theme_set(theme_cowplot())

ggplot(sp, aes(x = bmi_curr_xps, col = factor(age))) +
  stat_ecdf(linewidth = 1.3) +
  geom_segment(aes(x = 20.5, xend = 20.5, y = 0, yend = 0.5),
               col = "black", linewidth = 1, linetype = "dashed") +
  geom_segment(aes(x = 27.5, xend = 27.5, y = 0, yend = 0.5),
               col = "black", linewidth = 1, linetype = "dashed") +
  geom_segment(aes(x = 12, xend = 27.5, y = 0.5, yend = 0.5),
               col = "black", linewidth = 1, linetype = "dashed") +
  xlab("BMI (kg/m²)") +
  ylab("Percentile") +
  labs(col = "Age") +
